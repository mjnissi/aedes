function t1_rho(DATA,ROI,AddInfo)
...
[fname,fpath,findex]=uiputfile({'*.t1r;*.T1R;*.s1r;*.S1R',...
                    'T1R-Files (*.t1r, *.s1r)';...
                    '*.*','All Files (*.*)'},...
...
try
  aedes_fitmaps(DATA,'t1r',fit_vals,'FileName',[fp,filesep,fn]);
catch
  errordlg({'Could not calculate T1 rho maps. The following error was returned',...
           '',lasterr},'modal')
end
